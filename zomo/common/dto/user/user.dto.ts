...
import { CompanyTypesDto } from '../company/companytypes.dto';
import { LanguagesDto, RoleDto } from '../master';
...
export class UserDto {
    ...
    @Expose() preferred_lang: string;
    @Expose() status: number;
    // @Expose() refresh_token: string;
    @Expose() is_aro_build: number;
    ...
    @Expose()
    @Type(() => Number)
    @Transform(({ obj }) => (obj.location ? obj.location : null), {
        toClassOnly: true,
    })
    location_id: number;
    @Expose()
    ...
    companysetting: CompanySettingsDto;
    @Expose()
    @Type(() => DepartmentsDto)
    @Transform(({ value }) => {
        if (value) {
    ...
    preferred_language: LanguagesDto;
}
