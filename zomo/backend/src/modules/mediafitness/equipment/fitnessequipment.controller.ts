...
export class FitnessEquipmentController {
    ...
    async create(@Req() req: Request, @Res() res: Response, @Body() postData: CreateMediaFitnessEquipmentInput) {
        try {
            ...
            if(postData?.name){
                let title = `fitness_equipment_${resultedData['id']}`
                dynamicData[`${title}`]= postData?.name;
            }                      
            await this.translatorService.DynamicEngJsonData('Media',postData?.org_id,dynamicData,'Edit','Fitnessvideos','equipment');
    ...
    async update(@Req() req: Request, @Res() res: Response, @Body() postData: CreateMediaFitnessEquipmentInput) {
        try {
            ...
            let dynamicData = Object.create(null);
            if(postData?.name){
                let title = `fitness_equipment_${recordDetails['id']}`
                dynamicData[`${title}`]= postData?.name;
            }                      
            await this.translatorService.DynamicEngJsonData('Media',postData?.org_id,dynamicData,'Edit','Fitnessvideos','equipment');
