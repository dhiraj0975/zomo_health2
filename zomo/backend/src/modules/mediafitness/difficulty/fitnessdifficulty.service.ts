import { appConstant, CommonArrayService, CommonFileService, MediaFitnessDifficultyEntity } from '@common-constants';
import { Injectable } from "@nestjs/common";
import { InjectRepository } from "@nestjs/typeorm";
import { Repository } from "typeorm";
import { PaginationWithMediaFitnessInput } from '../input';
@Injectable()
export class FitnessDifficultyService {
    constructor(
        @InjectRepository(MediaFitnessDifficultyEntity, appConstant.READ_REPLICA.toLowerCase())
        private readonly readReplicaFitnessDifficultyRepository: Repository<MediaFitnessDifficultyEntity>,
        @InjectRepository(MediaFitnessDifficultyEntity, appConstant.MAIN.toLowerCase())
        private readonly writeReplicaFitnessDifficultyRepository: Repository<MediaFitnessDifficultyEntity>,
    ...
    async update(condition: any, data: any) {
        ...
        return await this.writeReplicaFitnessDifficultyRepository.createQueryBuilder('fitness')
            .update(MediaFitnessDifficultyEntity)
            .set(data)
            .where(condition)
            .execute();
    }
