...
export class FrontService {
    ...
    async getCampaignData(postData: any, req: Request = null, fields: any = ['campaign.id', 'campaign.campaign_name', 'campaign.tab_titled', 'campaign.tab_order']) {
        try{
            ...
            if (call_from === 2 || call_from === 12) { /* campaign summary api call */
                if(call_for === 'current') {
    ...
    async campaignDetailsData(fields: any[] = [],condition: any, orderBy: any = null, joinCondition: any[] = [],dataType: any = 'getOne') {
        try{
            if (!orderBy) {
                orderBy = { 'ca.id' : 'DESC' };
            }
            let query: any = this.readReplicacampaignRepository.createQueryBuilder('ca')
    ...
    async calculate_frequency(type:any = 'normal', tempPointsFL:any = [], extraDataArray:any = []){
        try{
            ...
            let frequincy_max_point = actDatas['frequincy_max_point'] || 0;
            let point_for_each = actDatas['point_for_each'] || 0;
            let marc = actDatas['min_act_req_camp'] || 0;
    ...
    async campaignActivityData(fields: any[] = [],condition: any, orderBy: any = null, joinCondition: any[] = [],dataType: any = 'getOne') {
        try{
            if (!orderBy) {
                orderBy = { 'campaign_activity.id' : 'DESC' };
            }
            let query: any = this.readReplicaCampaignActivityRepository.createQueryBuilder('campaign_activity')
            if(joinCondition && joinCondition?.length > 0){
                for(let i = 0; i < joinCondition?.length; i++){
                    const { join_table, table, alias, on_condition, join_type } = joinCondition[i];
                    switch (join_type) {
