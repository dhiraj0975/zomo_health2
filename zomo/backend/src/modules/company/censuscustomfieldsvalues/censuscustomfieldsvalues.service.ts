import { appConstant, CensusCustomFieldsValuesEntity, CommonFileService } from '@common-constants';
...
export class CensusCustomFieldsValuesService {
    constructor(
        @InjectRepository(CensusCustomFieldsValuesEntity, appConstant.READ_REPLICA.toLowerCase())
        private readonly readReplicaCensusCustomFieldsValuesRepository: Repository<CensusCustomFieldsValuesEntity>,
        @InjectRepository(CensusCustomFieldsValuesEntity, appConstant.MAIN.toLowerCase())
    ...
    async update(condition: any, data: any) {
        data = await this.commonFileService.filterDataByEntityColumns(data, this.writeReplicaCensusCustomFieldsValuesRepository.metadata);
        return await this.writeReplicaCensusCustomFieldsValuesRepository.createQueryBuilder('cfv')
    ...
    async delete(condition: any){
        await this.writeReplicaCensusCustomFieldsValuesRepository.delete(condition);
    }
    ...
    async findDistinctUserIds(conditionCensu: string[]): Promise<number[]> {
        ...
        const queryBuilder = this.readReplicaCensusCustomFieldsValuesRepository.createQueryBuilder('census');
    ...
    async listRecordReport(fields: any, condition: any, orderBy: any = null) {
        ...
        const data = await this.readReplicaCensusCustomFieldsValuesRepository.find({
            where: condition,
            order: orderBy,
        });
