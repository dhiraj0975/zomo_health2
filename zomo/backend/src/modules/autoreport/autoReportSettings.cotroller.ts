import { appConstant, AutoreportsettingsDto, CommonArrayService, CommonDateService, CommonService, reportFieldsConstant, tableConstant } from '@common-constants';
import {
    Body,
    Controller,
    HttpException,
...
export class AutoReportSettingController {
    ...
    async getOne(@Req() req: Request, @Res() res: Response, @Body() postData: GetOneAutoReportInput) {
        try {
            ...
            let incentiveIDS = [], myplanIDS = [], challengeIDS = [], quicklinkIDS = [], fitnessvideoIDS = [], eventsIDS = [], quizIDS = [], RemIDS = [], RemTypeIDS = [],
                ...
                timeZoneData = [], departmentData = [], locationData = [], userIDS = [], userNames = [];
            if (autoReportDetails) {
                if (autoReportDetails.f_module_items && autoReportDetails.f_module_items != '' && autoReportDetails.f_module_items != '0') {
                    ...
                    if (autoReportDetails.module_id == '10') {
                        let IidS = autoReportDetails.f_module_items.split(",");
                        userIDS = userIDS.concat(IidS);
                    }
                ...
                if (userIDS.length > 0) {
                    ...
                    let userResults = await this.userService.findAllUserRecord(
                        {
                            status: 1,
                            id: In(userIDS)
                        },
                        // ["user.id AS id", "user.first_name AS first_name", "user.last_name AS last_name", "user.code AS code"],
                        ['id', 'first_name', 'last_name', 'code']
                    );
                    userNames = userResults?.map(user => ({
                        id: user.id,
                        code_full_name: `${user.code} - ${user.first_name} ${user.last_name}`
                    })) || [];
                }
                if (quizIDS.length > 0) {
                    quizIDS = [...new Set(quizIDS)]
            ...
            if (autoReportDetails?.f_from_date && autoReportDetails?.f_from_date != '' && autoReportDetails?.f_from_date != null) {
                let fromDate = autoReportDetails.f_from_date;
                autoReportDetails.f_from_date = this.commonDateService.normalizeDate(fromDate, 'MMM D, YYYY', 'MM-DD-YYYY');
                if (autoReportDetails?.f_from_date == 'Invalid date') {
                    autoReportDetails.f_from_date = this.commonDateService.normalizeDate(fromDate, 'MMM D, YYYY', 'DD-MM-YYYY');
                    if (autoReportDetails?.f_from_date == 'Invalid date') {
                        autoReportDetails.f_from_date = null;
                    }
                }
            }
            if (autoReportDetails?.f_to_date && autoReportDetails?.f_to_date != '' && autoReportDetails?.f_to_date != null) {
                let toDate = autoReportDetails.f_to_date;
                autoReportDetails.f_to_date = this.commonDateService.normalizeDate(toDate, 'MMM D, YYYY', 'MM-DD-YYYY');
                if (autoReportDetails?.f_to_date == 'Invalid date') {
                    autoReportDetails.f_to_date = this.commonDateService.normalizeDate(toDate, 'MMM D, YYYY', 'DD-MM-YYYY');
                    if (autoReportDetails?.f_to_date == 'Invalid date') {
                        autoReportDetails.f_to_date = null;
                    }
                }
            }
    ...
    async create(
        ...
        try {
            ...
            reportData['status'] = postData?.status || 1
            reportData['f_engagement_report'] = 0
            let allDepartmentIds = this.commonArrayService.transformToArray(postData?.f_department, ',');
            reportData['f_department'] = (allDepartmentIds && allDepartmentIds.length) ? allDepartmentIds.join(',') : null
            let allLocationIds = this.commonArrayService.transformToArray(postData?.f_location, ',');
            reportData['f_location'] = (allLocationIds && allLocationIds.length) ? allLocationIds.join(',') : null
            let allCountryIds = this.commonArrayService.transformToArray(postData?.f_country, ',', 'string');
            reportData['f_country'] = (allCountryIds && allCountryIds.length) ? allCountryIds.join(',') : null
            let allStateIds = this.commonArrayService.transformToArray(postData?.f_state, ',', 'string');
            reportData['f_state'] = (allStateIds && allStateIds.length) ? allStateIds.join(',') : null
            let allCityIds = this.commonArrayService.transformToArray(postData?.f_city, ',', 'string');
            reportData['f_city'] = (allCityIds && allCityIds.length) ? allCityIds.join(',') : null
            ...
            if (postData?.f_from_date && postData?.f_from_date !== '' && postData?.f_from_date !== null && postData?.f_from_date !== undefined) {
                postData.f_from_date = this.commonDateService.normalizeDate(postData?.f_from_date, 'MM-DD-YYYY');
            }
            if (postData?.f_to_date && postData?.f_to_date !== '' && postData?.f_to_date !== null && postData?.f_to_date !== undefined) {
                postData.f_to_date = this.commonDateService.normalizeDate(postData?.f_to_date, 'MM-DD-YYYY');
            }
            if (moduleId === 1) {
                ...
                if (postData?.report_fields) {
                    const reportFieldsJson = this.getFilteredReportFieldsJson(postData?.report_fields, ReportIncDefaultFields);
                    if (reportFieldsJson) {
                        reportData['report_fields'] = reportFieldsJson;
                    }
                }
            }
            else if (moduleId === 2) {
                ...
                reportData['report_fields'] = null;
                if (postData?.report_fields) {
                    const reportFieldsJson = this.getFilteredReportFieldsJson(postData?.report_fields, ReportMypDefaultFields);
                    if (reportFieldsJson) {
                        reportData['report_fields'] = reportFieldsJson;
                    }
                }
                ...
                reportData['report_fields'] = null;
                if (postData?.report_fields) {
                    const reportFieldsJson = this.getFilteredReportFieldsJson(postData?.report_fields, ReportChallengeDefaultFields);
                    if (reportFieldsJson) {
                        reportData['report_fields'] = reportFieldsJson;
                    }
                }
                ...
                reportData['report_fields'] = null;
                if (postData?.report_fields) {
                    const reportFieldsJson = this.getFilteredReportFieldsJson(postData?.report_fields, ReportQuicklinkDefaultFields);
                    if (reportFieldsJson) {
                        reportData['report_fields'] = reportFieldsJson;
                    }
                }
                ...
                reportData['report_fields'] = null;
                if (postData?.report_fields) {
                    const reportFieldsJson = this.getFilteredReportFieldsJson(postData?.report_fields, ReportFitnessVideoDefaultFields);
                    if (reportFieldsJson) {
                        reportData['report_fields'] = reportFieldsJson;
                    }
                }
                ...
                reportData['report_fields'] = null;
                if (postData?.report_fields) {
                    const reportFieldsJson = this.getFilteredReportFieldsJson(postData?.report_fields, ReportBillboardsDefaultFields);
                    if (reportFieldsJson) {
                        reportData['report_fields'] = reportFieldsJson;
                    }
                }
                ...
                reportData['report_fields'] = null;
                if (postData?.report_fields) {
                    const reportFieldsJson = this.getFilteredReportFieldsJson(postData?.report_fields, ReportChaDefaultFields);
                    if (reportFieldsJson) {
                        reportData['report_fields'] = reportFieldsJson;
                    }
                }
    ...
    async update(
        ...
        try {
            ...
            if (postData?.status) {
                ...
                reportData['status'] = recordDetails?.status;
            }
            reportData['f_engagement_report'] = 0;
            ...
            if (moduleId === 1) {
                ...
                const otherOptions = {
                    deviced_type: postData?.deviced_type || null
                };
                reportData['otheroptions'] = JSON.stringify(otherOptions);
            }
            else if (moduleId === 8) {
    ...
    getFilteredReportFieldsJson(
        requestedFields: string | string[] | undefined | null,
        ...
        } else if (typeof requestedFields === 'string') {
            fields = requestedFields
                .split(',')
                .map(s => s.trim())
                .filter(Boolean);
        ...
        const upperRequested = new Set(fields.map(f => f.toUpperCase()));
        const matched = Object.fromEntries(
            Object.entries(defaultFields).filter(([key]) => upperRequested.has(key.toUpperCase()))
        );
        return Object.keys(matched).length > 0 ? JSON.stringify(matched) : undefined;
    }
}
