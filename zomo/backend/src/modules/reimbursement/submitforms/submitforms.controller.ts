...
import { appConstant, CommonArrayService, CommonDateService, CommonFileService, CommonService, ReimbursementSubmitFormsDto, tableConstant } from '@common-constants';
import {
    Body,
    Controller,
    HttpException,
...
import {
    CreateReimbursementSubmitFormInput,
    DeleteReimbursementFormInput,
    ListReimbursementSubmitFormInput,
    PaginationReimbursementSubmitFormInput,
    UpdateReimbursementSubmitFormInput
} from '../../../input';
...
@Controller('reimbursement/submit-forms')
...
export class SubmitFormsController {
    ...
    @Post('paginate')
    async paginate(@Req() req: Request, @Res() res: Response, @Body() postData: PaginationReimbursementSubmitFormInput) {
        try {
            ...
            /* role.id 11 orgadmin*/
            if (req.tokenUser?.role_id == appConstant.ROLE.ORGADMIN || 1 && postData?.org_id) {
            ...
            resultedData['list'] = <any>(await this.commonArrayService.formatToDto(ReimbursementSubmitFormsDto, resultedData['list'], req.lang));
    ...
    async create(@Req() req: Request, @Res() res: Response, @Body() postData: CreateReimbursementSubmitFormInput, @UploadedFile() file: Express.Multer.File) {
        try {
            ...
            if (postData?.notes) {
                ...
                await this.translatorService.DynamicEngJsonData('Reimbursements', resultedData['org_id'], dynamicDatas, 'Add', 'SubmittedForms', resultedData['form_id'],resultedData['user_id']);
            }
            if (file && file.fieldname === 'attachments' && file.filename) {
                ...
                file.filename = `reimburfiles/subforms/${postData?.org_id.toString()}/${postData?.user_id.toString()}/${this.commonService.generateMD5(resultedData['id'].toString())}.${file.originalname.split('.')[file.originalname.split('.').length - 1]}`;
    ...
    async update(@Req() req: Request, @Res() res: Response, @Body() postData: UpdateReimbursementSubmitFormInput, @UploadedFile() file: Express.Multer.File) {
        try {
            ...
            if (file && file.fieldname === 'attachments' && file.filename) {
                file.originalname = this.commonFileService.formatFileName(file.originalname);
                file.filename = `reimburfiles/subforms/${postData?.org_id.toString()}/${postData?.user_id.toString()}/${this.commonService.generateMD5(recordDetails['id'].toString())}.${file.originalname.split('.')[file.originalname.split('.').length - 1]}`;
                await lastValueFrom(this.commonMicroservice.send({ cmd: 'upload_file' }, { path: path.resolve(file.path), filename: file.filename, userBucket: 'private' }));
                postData['attachments'] = file.filename;
            }
            if (postData?.activity_date) {
            ...
            if (companyData['health_form_mail'] == 1 && postData?.status && postData?.status != recordDetails.status && [appConstant.ROLE.ADMIN, appConstant.ROLE.ORGADMIN, appConstant.ROLE.GLOBALDATAMANAGER, appConstant.ROLE.WCH, appConstant.ROLE.GLOBALCLIENTENGAGEMENTMANAGER].includes(req.tokenUser?.role_id)) {
                let form_title = '';
                let subject = '';
            ...
            if (postData?.decline_reason) {
                ...
                await this.translatorService.DynamicEngJsonData('Reimbursements', recordDetails?.org_id, dynamicDatas, 'Add', 'SubmittedForms', recordDetails['form_id'],recordDetails['user_id']);
            }
            this.activityLogService.create(recordDetails, postData, tableConstant.REIMBURSEMENT.TBL_RE_SUBMITTED_FORMS, req.tokenUser?.id);
            let message = await this.translatorService.frontendReadTranslation(req.lang, "MSG_FORM_SUBMITTED_SUCCESSFULLY");
            if (this.commonService.isValidNumber(postData?.status)) {
                ...
                this.addNotification({
                    ...
                    title: 'Reimbursement ' + message.replace(' successfully',''),
                    message: `Your ${recordDetails?.['createForm']?.title} ` + message.replace(' successfully',''),
                    logo, 
                    url: `https://${process.env.DOMAIN}/reimbursements?tab=1?formId=${recordDetails?.['id']}`,
                    type: 'update',
                }, req);
            }
            return res.status(HttpStatus.OK).json({
    ...
    async delete(@Req() req: Request, @Res() res: Response, @Body() postData: DeleteReimbursementFormInput) {
        try {
            ...
            if (req.tokenUser?.role_id == appConstant.ROLE.REGISTERED) {
                ...
                this.activityLogService.create(recordDetails, { notes: recordDetails }, tableConstant.REIMBURSEMENT.TBL_RE_SUBMITTED_FORMS, req.tokenUser?.id, 'delete');
            } else {
                await this.submitFormsService.update(where, { deleted: '1' });
                this.activityLogService.create(recordDetails, { deleted: 1 }, tableConstant.REIMBURSEMENT.TBL_RE_SUBMITTED_FORMS, req.tokenUser?.id, 'delete');
            }
            return res.status(HttpStatus.OK).json({
                statusCode: 200,
                success: 1,
    ...
    @UseGuards(AccessGuard)
    @Post('list')
    async list(@Req() req: Request, @Res() res: Response, @Body() postData: ListReimbursementSubmitFormInput) {
        try {
            if (!postData?.user_id || !postData?.form_id || !postData?.org_id) {
                throw new Error(await this.translatorService.frontendReadTranslation(req.lang, "ERR_REQUIRED_PARAM_MISSING"));
            }
            let condition;
            condition = { status: Not('5') };
            postData = { ...postData, ...condition }
            let resultedData = await this.submitFormsService.listRecord(["sf.id", "sf.activity_id", "sf.activity_date", "sf.attachments", "sf.deleted"], { ...postData });
            resultedData = <any>(
                await this.commonArrayService.formatToDto(ReimbursementSubmitFormsDto, resultedData, req.lang)
            );
            return res.status(HttpStatus.OK).json({
    ...
    async multipleApprove(@Req() req: Request, @Res() res: Response, @Body() postData: any) {
        try {
            ...
            for (let record of postData?.id.split(',')) {
                ...
                this.activityLogService.create(recordDetails, saveData, tableConstant.REIMBURSEMENT.TBL_RE_SUBMITTED_FORMS, req.tokenUser?.id);
                const companyData = await this.companySettingsService.findOne({ org_id: recordDetails['org_id'] });
                if (companyData['health_form_mail'] == 1 && [appConstant.ROLE.ADMIN, appConstant.ROLE.ORGADMIN, appConstant.ROLE.GLOBALDATAMANAGER, appConstant.ROLE.WCH, appConstant.ROLE.GLOBALCLIENTENGAGEMENTMANAGER].includes(req.tokenUser?.role_id)) {
                    let form_title = '';
                    let toEmail = recordDetails['user']['email'];
                    let emailDetails = Object.create(null);
                    let activity = await this.activityService.findOne(`activity.id = ${recordDetails['activity_id']} AND activity.status != 2 AND activity.activity_display = 0`);
            ...
            return res.status(HttpStatus.OK).json({
                ...
                error: 0,
                data: null,
                message: await this.translatorService.frontendReadTranslation(req.lang, "MSG_FORM_APPROVED_SUCCESSFULLY"),
            });
        } catch (error) {
            this.activityLogService.error_log(req.tokenUser?.id,req?.originalUrl, error?.message, error, req);
            throw new HttpException(
    ...
    async translationList(@Req() req: Request, @Res() res: Response, @Body() postData: ListReimbursementSubmitFormInput) {
        try {
            if (!postData?.org_id) {
            ...
            if(postData?.org_id){
                ...
                joinTable.push(tableConstant.REIMBURSEMENT.TBL_RE_CREATE_FORMS);
            ...
            resultedData = <any>(await this.commonArrayService.formatToDto(ReimbursementSubmitFormsDto, resultedData, req.lang));
            if(postData?.org_id){
                if(postData?.form_id){
        ...
        try {
            if(formData?.type == 'add' || formData?.type == 'update'){
                ...
                let notificationData = {
                    ...
                    type: 2,
                    module_name: 'Reimbursment form',
                    submodule_name: 'Reimbursment form',
                    metadata: {
                        id: formData?.id,
