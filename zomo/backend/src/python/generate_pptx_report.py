...
def _create_table_slide(prs, data):
    MAX_ROWS_PER_SLIDE = 10  # adjust based on font/slide size
    ...
    if headers and rows_data:
        ...
        for start in range(0, len(rows_data), MAX_ROWS_PER_SLIDE):
            ...
            table_shape = slide.shapes.add_table(
                rows, cols, Inches(0.5), Inches(1.07), Inches(9.0), Inches(0.35 * rows)
            )
...
def _create_stacked_bar_chart_slide(prs, data):
    ...
    if not categories:
        print("Warning: No categories provided for stacked bar chart slide.", file=sys.stderr)
        return slide
    chart_data.categories = [c.get('label') for c in categories]
    
    legends = data.get("legend", [])
    if not legends and categories:
        num_values = len(categories[0].get("values", []))
        legends = [f"Series {i+1}" for i in range(num_values)]

    for i, legend_item in enumerate(legends):
        series_values = []
        for c in categories:
            values = c.get('values', [])
            value = float(values[i]) if i < len(values) else 0.0
            series_values.append(value)
        chart_data.add_series(legend_item, tuple(series_values))

    chart_type = XL_CHART_TYPE.BAR_STACKED
    chart_shape = slide.shapes.add_chart(
        chart_type, Inches(0.5), Inches(1), Inches(9), Inches(4.5), chart_data
    )
    ...
    # Manually set axis for non-100% charts
    if chart_type == XL_CHART_TYPE.BAR_STACKED:
    ...
    # Apply series colors
    static_colors = ["#08AE50", "#FFD735", "#FE0000", "#D62728", "#9467BD"]
    for i, series in enumerate(chart.series):
        hex_color = static_colors[i % len(static_colors)]
...
def _create_hra_summary_slide(prs, data):
    ...
    p.text = data.get('title', 'HRA Summary Results')
    p.font.size = Pt(20)
    p.font.bold = True
    ...
    if headers and table_data:
        ...
        table_shape = slide.shapes.add_table(rows_count, cols_count, Inches(0.5), Inches(1.0), Inches(9.0), Inches(3.5))
        table = table_shape.table
        ...
        for col_idx, header in enumerate(headers):
            ...
            cell.text = header
            cell.fill.solid()
            cell.fill.fore_color.rgb = RGBColor(0x2D, 0x3C, 0x3F)  # dark header
            p = cell.text_frame.paragraphs[0]
            p.font.bold = True
            p.font.size = Pt(8)
            p.font.color.rgb = RGBColor(255, 255, 255)
        ...
        for row_idx, row_data in enumerate(table_data):
            bg_color = RGBColor(0xFF, 0xFF, 0xFF) if row_idx % 2 == 0 else RGBColor(0xF7, 0xF7, 0xF7)
    ...
    # --- FOOTER (sample size) ---
    footer_text = f"N = {data.get('userCount', '')} per year"
    footer_box = slide.shapes.add_textbox(Inches(0.5), Inches(4.7), Inches(3), Inches(0.3))
...
def _create_risk_factors_slide(prs, data):
    ...
    if user_count := data.get('userCount'):
        txBox = slide.shapes.add_textbox(Inches(0.7), Inches(4.0), Inches(2), Inches(0.3))
        p = txBox.text_frame.paragraphs[0]
        p.text = f"N = {user_count} per year"
        p.font.size = Pt(8)
        p.font.color.rgb = RGBColor(0x3B, 0x55, 0x60)
...
def _create_questions_slide(prs, data):
    ...
    fill.solid()
    fill.fore_color.rgb = RGBColor(30, 53, 58)  # Dark background
    ...
    p = slide.shapes.add_textbox(Inches(0.5), Inches(0.7), Inches(4), Inches(0.6)).text_frame.paragraphs[0]
    p.text, p.font.size, p.font.bold = data.get("title", "Questions & Comments?"), Pt(18), True
    p.font.color.rgb = RGBColor(255, 255, 255)

    # --- Contact Info ---
    contact_text = data.get("contactInfo", """
    ...
    p = slide.shapes.add_textbox(Inches(0.5), Inches(1), Inches(4), Inches(2.5)).text_frame.paragraphs[0]
    p.text, p.font.size = contact_text, Pt(10)
    p.font.color.rgb = RGBColor(255, 255, 255)
...
def generate_presentation(report_data, output_path):
    ...
    slide_generators = {
        ...
        "hraResultChat": _create_hra_results_chart_slide,
