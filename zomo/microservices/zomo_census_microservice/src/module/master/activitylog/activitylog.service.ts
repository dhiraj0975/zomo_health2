...
export class ActivityLogService {
    ...
    async create(
        ...
        try {
            const updatedValues = JSON.parse(JSON.stringify(updatedData));
            ...
            await lastValueFrom(
                this.client.send(
                    { cmd: 'error_log' },
                    {
                        ...
                        log: `${JSON.stringify(log?.stack)} query: ${JSON.stringify(log?.query)}`,
                        req: JSON.stringify(filteredRequest),
                    },
                ),
            );
        } catch (error) {
