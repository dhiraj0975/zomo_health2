...
elif sheet_naming == 'dept':
    sheet_names = ['Department', 'Individual Report']
elif sheet_naming == 'normal':
    sheet_names = ['Report']
elif sheet_naming == 'individual':
    sheet_names = ['User Details','Participant Results']
else:
...
with pd.ExcelWriter(excel_filename, engine='openpyxl') as writer:
    for idx, path in enumerate(data_paths):
        ...
        sheet_name = sheet_names[min(idx, len(sheet_names)-1)] if sheet_naming in ['group', 'team', 'dept', 'normal'] else sheet_names[idx]  # Excel sheet max length 31
        if typeGet == '2':
