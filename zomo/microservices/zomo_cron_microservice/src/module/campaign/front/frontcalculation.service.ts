...
export class FrontCalculationService {
    ...
    async getCampaignUserCalculation(call_from:any = '', rewardDatas:any = [], otherDatas:any = []) {
        try{
            let {
                totalUsers = 0,
            ...
            let rewardWiseUsers = {};
            let returnData = {};
            let rewardWiseUsersLatestOneComplete = {};
            for (let rewardData of rewardDatas) {
                let userList = [];
                if(!rewardWiseUsers[sk]){
                    rewardWiseUsers[sk] = {};
                }
                if(!rewardWiseUsersLatestOneComplete[sk]){
                    rewardWiseUsersLatestOneComplete[sk] = {};
                }
                ...
                if (rewardData['Campaignactivity'] && rewardData['Campaignactivity']?.length > 0) {
                    ...
                    for (let actData of rewardData['Campaignactivity']) {
                        ...
                        if (actData['users'] && Object.keys(actData['users']).length > 0) {
                            let dateWiseUser = {};
                            userList = [...userList,...Object.keys(actData['users'])];
                                ...
                                if(cUserVal['Total'] !== undefined){
                                    ...
                                    if (catData['required_by_user'] == 'Y') {
                                        if (cUserVal['Total'] >= (catData['quentity'] * catData['point_for_each'])) {
                                            complete += 1;
                                            completeLOneAll[cUserId] = cUserId;
                                            if (cUserVal['Role'] == 16) {
                                                completeS += 1;
                                            }
                ...
                if(call_from != 5){
                    if(rewardData['isDefaultReward'] != 1){
                        ...
                        rewardWiseUsers[sk]['userList'] = [...new Set(userList)];
                    }
                    returnData = rewardWiseUsers;
                }else{
                    rewardWiseUsersLatestOneComplete[sk]['completeLOneAll'] = completeLOneAll;
                    returnData = rewardWiseUsersLatestOneComplete;
                }
                sk += 1;
            }
            return returnData;
